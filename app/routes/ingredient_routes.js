// // Express docs: http://expressjs.com/en/api.html
// const express = require('express')
// // Passport docs: http://www.passportjs.org/docs/
// const passport = require('passport')

// // pull in Mongoose model for ingredients
// const Ingredient = require('../models/ingredient');
// const Recipe = require('../models/recipe');

// // this is a collection of methods that help us detect situations when we need
// // to throw a custom error
// const customErrors = require('../../lib/custom_errors')

// // we'll use this function to send 404 when non-existant document is requested
// const handle404 = customErrors.handle404
// // we'll use this function to send 401 when a user tries to modify a resource
// // that's owned by someone else
// const requireOwnership = customErrors.requireOwnership

// // this is middleware that will remove blank fields from `req.body`, e.g.
// // { ingredient: { title: '', text: 'foo' } } -> { ingredient: { text: 'foo' } }
// const removeBlanks = require('../../lib/remove_blank_fields')
// // passing this as a second argument to `router.<verb>` will make it
// // so that a token MUST be passed for that route to be available
// // it will also set `req.user`
// const requireToken = passport.authenticate('bearer', { session: false })

// // instantiate a router (mini app that only handles routes)
// const router = express.Router()

// // INDEX
// // GET /ingredients
// router.get('/ingredients', (req, res, next) => {
// 	Ingredient.find()
// 		.then((ingredients) => {
// 			// `ingredients` will be an array of Mongoose documents
// 			// we want to convert each one to a POJO, so we use `.map` to
// 			// apply `.toObject` to each one
// 			return ingredients.map((ingredient) => ingredient.toObject())
// 		})
// 		// respond with status 200 and JSON of the ingredients
// 		.then((ingredients) => res.status(200).json({ ingredients: ingredients }))
// 		// if an error occurs, pass it to the handler
// 		.catch(next)
// })

// // INDEX- User Specific
// // GET /ingredients/mine
// router.get('/ingredients/mine', requireToken, (req, res, next) => {
// 	Ingredient.find({ owner: req.user.id })
// 		.then((ingredients) => {
// 			// `ingredients` will be an array of Mongoose documents
// 			// we want to convert each one to a POJO, so we use `.map` to
// 			// apply `.toObject` to each one
// 			return ingredients.map((ingredient) => ingredient.toObject())
// 		})
// 		// respond with status 200 and JSON of the ingredients
// 		.then((ingredients) => res.status(200).json({ ingredients: ingredients }))
// 		// if an error occurs, pass it to the handler
// 		.catch(next)
// })

// // SHOW
// // GET /ingredients/5a7db6c74d55bc51bdf39793
// router.get('/ingredients/:id', requireToken, (req, res, next) => {
// 	// req.params.id will be set based on the `:id` in the route
// 	Ingredient.findById(req.params.id)
// 		.then(handle404)
// 		// if `findById` is succesful, respond with 200 and "ingredient" JSON
// 		.then((ingredient) => res.status(200).json({ ingredient: ingredient.toObject() }))
// 		// if an error occurs, pass it to the handler
// 		.catch(next)
// })

// // CREATE
// // POST /ingredients
// router.post('/ingredients', requireToken, (req, res, next) => {
// 	console.log('this is req.body', req.body)
// 	// set owner of new ingredient to be current user
// 	req.body.ingredient.owner = req.user.id

// 	Ingredient.create(req.body.ingredient)
// 		// respond to succesful `create` with status 201 and JSON of new "example"
// 		.then((ingredient) => {
// 			res.status(201).json({ ingredient: ingredient.toObject() })
// 		})
// 		// if an error occurs, pass it off to our error handler
// 		// the error handler needs the error message and the `res` object so that it
// 		// can send an error message back to the client
// 		.catch(next)
// })

// // UPDATE
// // PATCH /ingredients/5a7db6c74d55bc51bdf39793
// router.patch('/ingredients/:id', requireToken, removeBlanks, (req, res, next) => {
// 	// if the client attempts to change the `owner` property by including a new
// 	// owner, prevent that by deleting that key/value pair
// 	delete req.body.ingredient.owner

// 	Ingredient.findById(req.params.id)
// 		.then(handle404)
// 		.then((ingredient) => {
// 			// pass the `req` object and the Mongoose record to `requireOwnership`
// 			// it will throw an error if the current user isn't the owner
// 			requireOwnership(req, ingredient)

// 			// pass the result of Mongoose's `.update` to the next `.then`
// 			return ingredient.updateOne(req.body.ingredient)
// 		})
// 		// if that succeeded, return 204 and no JSON
// 		.then(() => res.sendStatus(204))
// 		// if an error occurs, pass it to the handler
// 		.catch(next)
// })

// // DESTROY
// // DELETE /ingredients/5a7db6c74d55bc51bdf39793
// router.delete('/ingredients/:id', requireToken, (req, res, next) => {
// 	Ingredient.findById(req.params.id)
// 		.then(handle404)
// 		.then((ingredient) => {
// 			// throw an error if current user doesn't own `ingredient`
// 			requireOwnership(req, ingredient)
// 			// delete the ingredient ONLY IF the above didn't throw
// 			ingredient.deleteOne()
// 		})
// 		// send back 204 and no content if the deletion succeeded
// 		.then(() => res.sendStatus(204))
// 		// if an error occurs, pass it to the handler
// 		.catch(next)
// })

// module.exports = router
